...
reference_frame_id = 0 # Tag used as the local reference frame
hinge_dist_tag_id = 3 # The tag used for the hinge distance
rot_dist_tag_id = 5 # The tag used for the rotation distance

neighbors_mode = "radius" # Choices: "knn" or "radius"
neighbors_k = 20
neighbors_rad = 80
target_dim = 2
...
video_fname = "./datasets/articulated_objects/tial_articulated_object_dataset/can_opener_2.mp4"
...
poses, frame_nums = tial.load_video(video_fname, calib, tag_size)
poses = poses[::downsample]
frame_nums = frame_nums[::downsample]
...
x_axis = np.array([geometry.quat_to_mat(pose_list[rot_dist_tag_id].quat)[0:2,0] for pose_list in poses])
rot_dist = np.arctan2(x_axis[:,1], x_axis[:,0])
...
# ax.set_xlabel("Can Opener Handle Angle")
# ax.set_ylabel("Can Opener Twist Angle")
...
def display_state(xy, embed_idx):
	...
	ax3.set_xlim(xlims)
	ax3.set_ylim(ylims)
	ax3.plot(points[[0,1],0], points[[0,1],1], c="black") # Show the connected parts
	ax3.plot(points[[2,3],0], points[[2,3],1], c="black") # Show the connected parts
	ax3.plot(points[[4,5],0], points[[4,5],1], c="black") # Show the connected parts
	ax3.scatter(orig_point[0::3], orig_point[1::3], c="orange") # Show the corners
	...
	for i in range(len(poses)):
		...
		ax3.plot(x[0,:], x[1,:], c="red")
		ax3.plot(y[0,:], y[1,:], c="green")
		ax3.plot(z[0,:], z[1,:], c="blue")

		ax3.text(t[0,0]+15, t[1,0]-5, str(i), fontsize="medium")
	fig.canvas.draw()
