...
dataset_func = synthetic_manifolds.make_RP2 # Change this
n_points = 2000
neighbors_k = 12
noise = 0
...
ax.scatter(data[:,0], data[:,1], data[:,2], c=param[:,param_idx], cmap=plt.get_cmap("hsv"), zorder=10000, vmin=0, vmax=1, lw=0.5)

def keypress(event):
	global param_idx
	if event.key == " ":
		param_idx = (param_idx + 1) % 2
...
a.seed_charts_iterative_farthest_point(100, 2)
...
ax.scatter(a.embeddings[chart_idx][:,0], a.embeddings[chart_idx][:,1], c=param[np.array(list(a.atlas[a.atlas_idx[chart_idx]])),param_idx], cmap=cmap, zorder=10000, vmin=0, vmax=1, lw=0.5)
...
def keypress(event):
	...
	ax.cla()
	if draw_ism:
		ax.scatter(raw_embedding[:,0], raw_embedding[:,1], c=param[:,param_idx], cmap=cmap, zorder=10000, vmin=0, vmax=1, lw=0.5)
	else:
		ax.scatter(a.embeddings[chart_idx][:,0], a.embeddings[chart_idx][:,1], c=param[np.array(list(a.atlas[a.atlas_idx[chart_idx]])),param_idx], cmap=cmap, zorder=10000, vmin=0, vmax=1, lw=0.5)
	fig.canvas.draw()
